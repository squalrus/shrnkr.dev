...
The shrink algorithm (`shrinkText()`, level names Garlic Press/Panini Press/Vacuum Sealer/Pressure Cooker/Meat Grinder for levels 1–5) lives in [js/shrink.js](js/shrink.js), a plain global script with no DOM dependency and an injectable `rng` parameter (defaults to `Math.random`), so it can be required from Node for tests without a browser. Pipeline stages, in order: redundant phrase detection (deletes pure filler like "to be honest"; shortens wordy-but-meaningful phrases like "in order to" → "to"), punctuation stripping, symbol substitution (common words → compact symbols, e.g. "move" → "→", "and" → "&"), adaptive vowel elision, then word merging — words start pairing up at level 2, and from level 3 on the entire response collapses into one lowercase word (no more spaces). Level 4 additionally runs DLE ("Duplicate Letter Elimination"), which collapses any run of identical consecutive characters down to one. Level 5 instead runs JDSL ("Just Drop Some Letters"), which deletes every 4th character of the final merged string outright regardless of what it is — DLE and JDSL are mutually exclusive, each level 4/5's own way of staying visibly different from the level below it.
...
- `node tests/compare-levels.js` — diagnostic report, not assertions. Prints every level's output for a set of sample texts side by side with a level 4 vs level 5 similarity score. DLE (level 4) and JDSL (level 5) are what keep levels 3/4/5 meaningfully different (level 4 vs 5 typically ~82–84% similar, notably shorter) instead of converging to near-identical output on typical input, which is what happened before JDSL existed (levels 4/5 previously only differed by `vowelKeepChance`, since merge-group size is a no-op once both join chunks with `''`).
...
- [js/shrink.js](js/shrink.js) — `shrinkText()`, the full compression pipeline (phrase detection, punctuation stripping, symbol substitution, vowel elision, word merging, level-4 DLE, level-5 JDSL — see Testing below for details). DOM-free and separately requireable so it's testable from Node.
...
- **Suite bar** — the `.site-topstack` wrapper at the top of `index.html`'s `<body>` holds a slim `.suite-bar` (links to squalr.us + sibling sites) stacked directly above `<header>`; the wrapper, not `<header>` itself, carries `position:sticky`. Kept pixel-identical (font, sizing, spacing, background/border/muted-text colors — all literal hex, not tokens) to badgefor.me's copy of the same component; only the squalr.us brand color is allowed to differ per site. Changing this bar here means changing it in badgefor.me too.
...
- **Accent:** `--green: #0E9A73` for decorative/UI-only uses (badge dot, borders, range accents, pricing-bar fill — all ≥3:1 against their backgrounds) and `--green-dark: #0B7D5E` for anything rendered as *text* (eyebrows, links, primary CTA background, labels) — `--green` itself fails WCAG AA (4.5:1) as text or as a background under white text, so don't reintroduce it for copy; `--green-tint: #E9F6F0` fills; `--rust: #D2543F` (fee portion of pricing bars); `--dark: #14161A` / `--dark-2: #0C0E11` (terminal/output surfaces, suite bar) with `--mint: #7FE3BC` output text, `--amber: #E3B77F` code accents, and `--dark-muted: #787C83` for de-emphasized text on dark surfaces (code comments, suite bar); `--ink-faint: #9AA0A6` fails AA on light backgrounds — only safe on dark surfaces (e.g. `.install-copy p`), use `--ink-soft` for faint text on light backgrounds instead
- **Type:** `Space Grotesk` for display headings (h1/h2/brand/stat values); `IBM Plex Sans` for body copy and buttons; `JetBrains Mono` (`.mono`) for labels, nav eyebrows, stats, code blocks, and the suite bar
- **Motifs:** sticky suite bar + blurred header stacked together (see Architecture); rounded (`12–24px`) card surfaces with soft shadows instead of hard borders; dark terminal-style output/code blocks with mint text; numbered (`01`–`06`) feature cards
- **Site width:** `.wrap` max-width is `1180px`, matching badgefor.me's `--maxw` — keep the two in sync
- **Responsive breakpoints:** 900px (nav links hide, CTA remains), 760px (testimonials 3→1 col, install block 2→1 col), 640px (demo/features/results 2→1 col, pricing row columns collapse), 560px (suite bar hides its descriptor/"+ more soon" text)
